			...
			<div class="row mt-3">
				...
				<div class="col-sm-6">
					...
					<?php else: ?>
						...
						<?php foreach ($parentCategoryData as $key => $parentCategory): ?>
							<option value="<?php echo $parentCategory->categoryId ?>"
							<?php if ($parentCategory->categoryId == $category->parentCategoryId):
	echo "selected";endif;?>>
				...
				<div class="col-sm-6">
					...
					<select name="category[status]" class="form-control">
						...
						<?php foreach ($category->getStatusOptions() as $key => $value): ?>
								<option value="<?php echo $key ?>" <?php if ($category->status == $key): ?> selected <?php endif;?>> <?=$value?></option>
						<?php endforeach;?>
					</select>
				</div>
			...
			<div class="row mt-3 mb-3">
				<div class="col-sm-6">
					<input type="button" onclick = "mage.setForm(document.getElementById('form')).load();" name="update" value="<?php if ($this->getRequest()->getGet('categoryId')) {echo 'Update';} else {echo 'Add';}?>" class="form-control btn btn-dark">
				</div>
				<div class="col-sm-6">
